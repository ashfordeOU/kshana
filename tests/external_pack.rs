...
use kshana::api::{ExternalPack, RunOutput, Scenario, ScenarioMeta, KshanaError};
...
fn register_into_makes_pack_buildable() {
    ...
    assert!(!reg.contains(&id), "demo id must be absent before register_into");
    ...
    assert!(reg.contains(&id), "register_into must install the demo factory");
    let scenario = reg
        .build(&id, "payload = 42")
...
fn register_into_default_is_noop() {
    ...
    assert_eq!(reg.ids().count(), 0, "defaulted register_into must be a no-op");
}
