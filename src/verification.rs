...
pub fn verification_matrix() -> Vec<VerificationItem> {
    ...
    vec![
        ...
        VerificationItem {
            ...
            capability: "Schur-complement marginal of the {lunocenter-X, scale} block of the 7-parameter Helmert datum Fisher: the scalar degeneracy metric λ_min(S), the origin-X CRLB, and the origin↔scale correlation",
            module: "lunar_identifiability",
            tests: "tests/lunar_datum_identifiability_reference.rs::decompose_matches_scipy_reference (16 synthetic 7×7 SPD matrices spanning well-conditioned to {0,3}-near-degenerate; Schur λ_min / origin CRLB / |corr| vs SciPy/NumPy to <1e-9 rel)",
            oracle: "Independent SciPy/NumPy reference (scripts/gen_datum_identifiability_ref.py → tests/fixtures/datum_identifiability/scipy_ref.csv): the Schur complement, its eigenvalues, and the 2×2 inverse computed by an independent library. Scoped to the decomposition LINEAR ALGEBRA; the lunar correlation/CRLB MAGNITUDES from real geometry remain MODELLED.",
            oracle_kind: ExternalDataset,
            status: Validated,
        },
        ...
        VerificationItem {
            ...
            capability: "Classification of the internal-ranging datum problem: a single range observation contributes rank 1 (6-dimensional null space); the origin-X↔scale pair is near-null and physical DE440 libration lifts the defect to zero while the pair stays near-degenerate (the structure Sośnica 2025 reports, r≈−0.97)",
            ...
            oracle: "Engine-reproduced geometric structure + the closed-form 2×2 Schur identity; consistent with Sośnica et al. 2025 (arXiv:2510.15484). The classification is structural; the correlation/CRLB magnitudes under real geometry are MODELLED (reflector coords + orientation held fixed).",
            oracle_kind: OracleKind::InternalConsistency,
            status: VerificationStatus::Modelled,
        },
        VerificationItem {
            requirement: "Multi-technique lunar datum information (LLR + lunar VLBI + orbiter range)",
            capability: "Datum-Jacobian rows for Earth–reflector LLR range, two-station lunar-VLBI differential delay, and orbiter-to-beacon range over the 7-parameter datum, combined into one consistently-preconditioned Fisher; demonstrates that off-radial (orbiter / depth-diverse) tracking breaks the origin-X↔scale degeneracy where transverse VLBI helps only indirectly",
        ...
        VerificationItem {
            requirement: "Cost-aware gauge-constrained datum experiment design (OED + Pareto frontier)",
            capability: "Budget-constrained selection of measurement campaigns maximising the degeneracy metric: a greedy heuristic validated against an exact brute-force oracle, and a cost/accuracy Pareto frontier over a representative LLR+VLBI+orbiter menu",
            module: "lunar_oed",
            tests: "lunar_oed::tests (greedy==exhaustive on a constructed menu; exhaustive ≥ greedy invariant; frontier monotone; radial diversity beats transverse)",
            oracle: "greedy validated against the exact exhaustive (brute-force) optimizer (internal). The optimizer is a Modelled decision tool; costs/precisions are representative, the frontier is a parameterized trade curve, not a mission recommendation (see NOTICE.md).",
            oracle_kind: OracleKind::InternalConsistency,
            status: VerificationStatus::Modelled,
        },
        ...
        VerificationItem {
            ...
            tests: "oem::tests (a symmetric PD covariance round-trips KVN→parse to f64 round-off and stays symmetric; the block emits exactly the 21 lower-triangular entries — i+1 per matrix row — with/without COV_REF_FRAME; a block with the wrong value count and a missing block are both rejected)",
            oracle: "Self-consistency of the CCSDS 502.0 covariance serialisation: the serialise→parse round-trip reproduces the matrix to f64 round-off, the lower-triangular ordering carries exactly 21 entries, and malformed blocks are rejected — internal-consistency checks against the module's own writer/reader, NOT an external dataset, so the row stays InternalConsistency. MODELLED format mapping — no cross-validation against a third-party CCSDS OEM covariance fixture",
            oracle_kind: InternalConsistency,
            status: Modelled,
        },
    ]
