...
const UNITS: &[(&str, &str, &str, Option<&str>)] = &[
    ("campaign.n_stations", "count", "input", Some("Earth stations in the observing network")),
    ("campaign.n_beacons", "count", "input", Some("lunar-surface beacons whose coordinates the campaign solves for")),
    ...
    ("campaign.arc_hours", "hr", "input", Some("total span of the observing schedule, first epoch to last")),
    ("campaign.step_min", "min", "input", Some("spacing between consecutive epochs of the schedule")),
    ("campaign.elevation_mask_deg", "deg", "input", Some("minimum elevation the Moon must clear at an Earth station, applied at BOTH ends of a baseline")),
    ...
    ("campaign.stations.lat_deg", "deg", "input", Some("geodetic latitude of the Earth station")),
    ("campaign.stations.lon_deg", "deg", "input", Some("geodetic longitude of the Earth station, east positive")),
    ("campaign.stations.alt_m", "m", "input", Some("height of the Earth station above the reference ellipsoid")),
    ...
    ("campaign.beacons.lon_deg", "deg", "input", Some("selenographic longitude of the beacon, east positive")),
    ("campaign.beacons.alt_m", "m", "input", Some("height of the beacon above the mean lunar radius")),
    ("campaign.beacons.observations", "count", "computed", Some("surviving (baseline, epoch) pairs that observe this beacon after both masks")),
    ...
    ("campaign.beacons.sigma_y_m", "m", "computed", Some("formal 1-sigma of this beacon's Moon-body-fixed y coordinate")),
    ("campaign.beacons.sigma_z_m", "m", "computed", Some("formal 1-sigma of this beacon's Moon-body-fixed z coordinate")),
    ...
    ("beacon_information.defect", "count", "computed", Some("unobservable directions in the beacon block: dimension minus rank")),
    ...
    ("helmert.rank", "count", "computed", Some("how many of the seven similarity parameters this campaign constrains")),
    ...
    ("helmert.rel_tol", "1", "input", Some("singular-value ratio below which a direction is counted unobservable")),
    ("helmert.unobservable_directions.index", "count", "computed", Some("position of this direction in the emitted list, not a parameter number")),
    ...
    ("datum_accuracy.rotation_sigma_urad", "urad", "computed", Some("per-axis formal 1-sigma of the three rotation parameters")),
    ("datum_accuracy.rotation_sigma_norm_rad", "rad", "computed", Some("Euclidean norm of the three per-axis rotation sigmas")),
    ("datum_accuracy.scale_sigma_ppb", "ppb", "computed", Some("formal 1-sigma of the single scale parameter")),
    ...
    ("comparison.assumed_isotropic.rotation_sigma_norm_rad", "rad", "modelled", Some("the SAME Helmert propagation run with one isotropic per-coordinate sigma in place of the campaign covariance, so only the error model differs")),
    ("comparison.assumed_isotropic.scale_sigma_ppb", "ppb", "modelled", Some("scale sigma under that same isotropic substitution")),
    ("comparison.assumed_isotropic.rank", "count", "computed", Some("rank of the design under the isotropic substitution; it shares the geometry, so it should match")),
    ...
    ("comparison.independence_discarded.rank", "count", "computed", Some("rank with the inter-beacon correlations zeroed, which changes weights but not geometry")),
    ...
    ("comparison.injected_transform_scenario.rotation_error_norm_rad", "rad", "modelled", Some("the lunar-frame-realisation scenario's RECOVERY ERROR against its injected transform — an error, not a formal sigma, and not comparable term for term")),
    ("comparison.injected_transform_scenario.scale_error_ppb", "ppb", "modelled", Some("that scenario's scale recovery error against its injected value")),
    ("comparison.injected_transform_scenario.rms_residual_m", "m", "modelled", Some("RMS fit residual of that scenario's own Helmert solve")),
    ...
    ("comparison.ratio_assumed_isotropic_over_campaign_scale", "1", "computed", Some("isotropic scale sigma divided by the campaign's: how much the error model alone moves the answer")),
