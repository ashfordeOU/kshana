...
//! scientific contribution. `VerificationStatus::Validated` covers exactly one thing: the
//! ROTATION-ONLY (6-parameter) decomposition of the real DE440 / INPOP21a / EPM2021
//! disagreement, checked against an independent SciPy SVD least-squares fit (see
//! `tests/lunar_interop_budget_reference.rs`). The 7-parameter [`helmert_fit`] below is NOT
//! covered by that external check — its evidence is the analytic-recovery unit test and
//! `tests/lunar_helmert_fit_cross_check.rs`.
...
//!
//! # Two `helmert_fit`s, and which to reach for
//! The crate carries a second, older 7-parameter fit,
//! [`crate::lunar_frame_realise::helmert_fit`]. They are not duplicates:
//!
//! - `lunar_frame_realise::helmert_fit` iterates Gauss–Newton on the EXACT model
//!   `q = t + (1+s)·R(θ)·p`, takes an isotropic σ, returns `None` on a degenerate geometry,
//!   and reports scale in parts per billion. Use it when the datum may be large or the
//!   geometry may be singular. It is `Validated` against an Umeyama-SVD oracle.
//! - [`helmert_fit`] here linearises ONCE at the zero datum and solves the centred normal
//!   equations by Cholesky with Jacobi preconditioning, unweighted, always returning a fit
//!   with a dimensionless scale. Use it for the nanoradian-scale inter-ephemeris datums it
//!   was built for, where one linearisation is exact to well below the signal — and where
//!   the centring and preconditioning are what keep the 8-order-of-magnitude column-scale
//!   spread from destroying the solve.
//!
//! They agree on translation and scale wherever the second-order term is negligible, but
//! their ROTATION VECTORS ARE NEGATIVES of one another: `lunar_frame_realise` builds its
//! rotation from the SOFA `iauRx/Ry/Rz` frame matrices (`q − p ≈ t + s·p − θ×p`) while this
//! module's [`crate::lunar_datum::apply_datum7`] uses the active Rodrigues rotation
//! (`q − p ≈ t + s·p + θ×p`). Each is self-consistent with its own forward model, and nothing
//! in the crate crosses between them — but a `rotation_rad` handed over as a `rot_rad` would
//! silently invert the datum. `tests/lunar_helmert_fit_cross_check.rs` pins the sign relation,
//! the agreement once it is applied, and the rotation magnitude at which agreement stops.
