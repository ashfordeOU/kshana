...
pack. The twenty-one scenario kinds (twenty named kinds plus the clock pack as the
historical default, in v0.17.0) span the resilient-PNT stack from terrestrial through
cislunar to deep space: clock holdover,
...
ephemeris/ground-track propagation, real-observation single-point positioning,
gravity-map, terrain-aided, and combined alternative-PNT navigation,
deep-space and Mars radiometric navigation (a simulated relay-constellation figure of
merit, not a certified protection level), and the one- and $N$-dimensional parameter
sweeps (\code{sweep},
...
instantiates. The clock curves, for example, are tagged with the optical-lattice,
...
provided in-browser SGP4. The point here is therefore not the browser target but that
