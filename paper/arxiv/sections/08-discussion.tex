...
buy, and over what operating envelope?} Instead of asserting a verdict, Kshana
...
discipline is an envelope rather than a marketing number: a region of the operating space
...
with outage duration. The break-even platform-vibration ASD rises from
$\approx\!334~\mu\mathrm{g}/\sqrt{\mathrm{Hz}}$ at a $30$~s outage to
$\approx\!2640~\mu\mathrm{g}/\sqrt{\mathrm{Hz}}$ near a
$1800$~s outage, so the cold-atom advantage is real for long, quiet holdovers and
erodes under aggressive platform vibration. The clock study is starker still: the deployed chip-scale reference crosses a
\SI{1}{\micro\second} timing budget at roughly \SI{8.5}{\hour} of holdover, while
the ground-lab optical-lattice and flight-qualified maser curves stay below that budget
across the full \SI{24}{\hour} sweep. These are performance-model projections from cited device
...
a crossover study or a propagated state can move into established pipelines such as RTKLIB
\citep{takasu2009}, GMAT \citep{nasa_gmat}, and Orekit \citep{orekit}, instead of
...
ahead. The measurement-to-position loop is now closed by a code single-point-positioning
path validated on real IGS observations; carrying it from code to carrier-phase precise
positioning --- ambiguity resolution and PPP/RTK --- is the consequential next step on
the receiver-processing side. The spoofing detector is characterised against published TEXBAT
...
honestly. Across the paper the same discipline holds: force-model fits are labelled as
fits and not orbit determination, the favourable reduced-dynamic residuals are reported
as bounds rather than measures, and the LRO miss is stated above its target instead of
being tuned away. Every quantum figure is a projection and every spoofing claim a parameter-level
characterisation (Sections~\ref{sec:validation}--\ref{sec:limitations}). To our knowledge
no open tool runs this full integrated PNT-resilience stack with these honesty controls
attached. That stance (validated-or-not-modeled stated per figure-of-merit, the
