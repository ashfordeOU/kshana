...
We treat the following disclosures not as caveats appended after the fact but as
the honesty contract the methodology in Section~\ref{sec:method} demands: every figure
of merit is reported alongside the strength of the evidence that backs it, and a
result that is a model projection or a force-model fit is never dressed as a
measurement. The same discipline applies to the boundaries of the engine itself.
...
\begin{itemize}
  ...
  (Figure~\ref{fig:clock}). Where a like-for-like comparison is not possible we say so:
  the optical time-transfer floor excludes the non-reciprocal systematics that limit
  real two-way links, whereas the RF figure includes them.
  ...
  fitting} --- propagation-overlap consistency against an external truth product ---
  not by processing observations. The headline is the full-arc Galileo result,
  \SI{0.61}{\metre} over \SI{24}{\hour} at gravity-field degree/order~70. The
  reduced-dynamic tier (Galileo \SI{0.13}{\metre}, Swarm \SI{0.10}{\metre}) admits
  ...
  \item \textbf{LRO is above target, stated plainly.} The LRO reduced-dynamic
  residual is \SI{6.6}{\metre}, above our \SI{5}{\metre} target. Swapping the default
  ephemeris for a DE440/ANISE realization leaves it essentially unchanged
  ...
  scoped to the LRO arc only --- not a discovery --- and we credit \texttt{nyx}/ANISE
  ...
  \item \textbf{Kshana does not process real observations or compute a PVT fix.}
  It is a resilience-comparison engine, not a receiver or an estimator over
  measurements; it answers a different question from tools such as RTKLIB
  \citep{takasu2009} (Table~\ref{tab:intersection}). There is no
  measurement-to-position pipeline.
...
behind it --- and, just as importantly, what it does not: it is not a measurement
