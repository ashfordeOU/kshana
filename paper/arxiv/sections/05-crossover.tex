...
sensor outperforms its classical counterpart. For the lever whose operating space
warrants it---the inertial sensor---it is a \emph{map}: a grid of operating points
across the realistic published-parameter envelope, at every node of which we report
the resilience figure of merit as a distribution rather than a number, and against
which we locate the \emph{break-even} contour where the quantum advantage disappears.
The clock and time-transfer levers, whose outcome turns on device maturity rather
than a two-axis trade-off, are reported in the same spirit---as a maturity-aware
holdover ladder and a like-for-like link budget. The question this study
...
$\sqrt{S_a/(3T)}$ in the acceleration power spectral density $S_a$ (in
$(\mathrm{m/s^2})^2/\mathrm{Hz}$) and interrogation time $T$. We plot and tabulate the
corresponding amplitude spectral density $\mathrm{ASD}=\sqrt{S_a}$ in
$\mu\mathrm{g}/\sqrt{\mathrm{Hz}}$, the conventional vibration-specification unit. The
classical leg uses the NaveGo-validated inertial error profile
...
\SI{1800}{\second}, and the platform-vibration acceleration noise as an amplitude
spectral density (ASD), from $1~\mu\mathrm{g}/\sqrt{\mathrm{Hz}}$ (an isolated,
quiescent platform) to roughly $3~\mathrm{mg}/\sqrt{\mathrm{Hz}}$ (a noisy host). At
each node the figure of merit is the \emph{ensemble mean}, over the 64 seeds, of the
per-run 95th-percentile horizontal position error, with a 95\% bootstrap confidence
interval on that mean; the advantage is the ratio of the classical to the quantum
ensemble mean. Figure~\ref{fig:inertial} shows the full map.
...
\begin{figure}[t]
  ...
  (\citealp{groves2013}), dead-reckoning position error (the ensemble mean of the
  per-run 95th percentile) as a
  function of outage duration and platform-vibration acceleration ASD
  ($\sqrt{S_a}$, in $\mu\mathrm{g}/\sqrt{\mathrm{Hz}}$). Each node is
  ...
  and erodes with vibration; the break-even ASD rises with outage duration. The
  quantum leg is a performance-model projection from laboratory-maturity device
  coefficients, not a measurement.}
  \label{fig:inertial}
\end{figure}

On an isolated platform the cold-atom advantage is large
...
\SI{2.5}{\milli\metre} versus navigation-grade \SI{0.75}{\metre}), rising to $439$
at \SI{60}{\second} and to $1915$ at
\SI{1800}{\second}, where the navigation-grade error has grown to
\SI{2.3}{\kilo\metre} while the cold-atom error is near \SI{1.2}{\metre}. The
confidence intervals are tight relative to this gap (the cold-atom ensemble mean
at the \SI{30}{\second}, $1~\mu\mathrm{g}/\sqrt{\mathrm{Hz}}$ node has a
bootstrap interval of $[2.0,\,2.9]~\si{\milli\metre}$), so the advantage on a
quiet platform is not a sampling artefact.

This advantage is nonetheless conditional, and the map shows exactly
where it fails. As the vibration ASD rises the cold-atom error grows steeply, driven
...
better choice, and the map reports this rather than only the favourable
corner.

These two findings connect through the break-even, which rises
...
\begin{table}[t]
  \centering
  \caption{Break-even platform-vibration ASD ($\sqrt{S_a}$) at which the
  navigation-grade unit matches the cold-atom sensor, as a function of outage
  duration. The break-even rises monotonically: a longer outage tolerates more
  vibration before the quantum advantage is lost.}
  \label{tab:breakeven}
  \small
  \begin{tabular}{rr}
    \toprule
    Outage duration (s) & Break-even ASD ($\mu\mathrm{g}/\sqrt{\mathrm{Hz}}$) \\
    \midrule
    30   & 334  \\
    60   & 448  \\
    120  & 724  \\
...
ensemble-mean 95th-percentile holdover timing error against the \SI{1}{\micro\second}
threshold; each rung is a 32-seed ensemble with a 95\% bootstrap interval.
Figure~\ref{fig:clock} shows the three curves.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\linewidth]{clock.pdf}
  \caption{Clock holdover ladder. Ensemble-mean 95th-percentile timing error against a
...
RMS of $\approx$\SI{0.29}{\milli\metre} (non-reciprocity included, a
\SI{1}{\pico\second} on-orbit projection target rather than a measured link budget)
against $\approx$\SI{341}{\milli\metre} for the RF link, still a
