...
Each row is checked against an **independent external oracle** (real dataset, independent reference implementation, or published reference vectors) and re-checked in CI. [Full 168-row matrix →](#validation-at-a-glance)
...
| ✅ | Global navigation satellite system (GNSS) dilution of precision (DOP) · machine-learning detector metrics | to **1e-6** · to **1e-9** | gnss_lib_py · scikit-learn |
...
> **Status: v0.27.2 · a validated, reproducible simulation substrate for PNT resilience.**
...
> | Deep-space / Mars navigation | **Simulation-validated** — synthetic closed-loop orbit determination (OD) + analytic self-consistency; Sun-central dynamics cross-checked vs JPL **DE440** (137 m @ 1-day arc) |
> | Real-mission deep-space OD | **Roadmap** — pending real Deep Space Network (DSN) / ESTRACK tracking-data validation |
>
...
assistant or agent**. It exposes six tools — `run_scenario`, `list_scenario_kinds`,
`validate_scenario`, `export_sp3`, `export_omm` and `export_oem` (each a thin wrapper
over `kshana::api`).
