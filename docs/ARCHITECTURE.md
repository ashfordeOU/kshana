...
```mermaid
flowchart TD
    api["api.rs — run_toml / run_scenario · 61 kinds"]
...
```mermaid
flowchart TD
    F["api::run_toml(src) · run_scenario(src)"] --> K{"ScenarioKind::classify<br/>typed · exhaustive · 61 kinds<br/>(absent/unknown kind → clock)"}
...
WebAssembly module backs the browser playground in `web/` and exports twelve
functions: `run`, `chart_svg`, `summary`, `table_csv`, `list_kinds`, `error_kind`, `version`, the
...
flowchart LR
    ...
    wasm["WebAssembly · wasm.rs (wasm-bindgen)<br/>run · chart_svg · summary · table_csv · list_kinds · error_kind · version · encode/decode_permalink<br/>export_sp3 · export_omm · export_oem"] --> api
