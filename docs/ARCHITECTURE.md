...
```mermaid
flowchart TD
    api["api.rs — run_toml / run_scenario · 48 kinds"]
...
```mermaid
flowchart TD
    F["api::run_toml(src) · run_scenario(src)"] --> K{"ScenarioKind::classify<br/>typed · exhaustive · 48 kinds<br/>(absent/unknown kind → clock)"}
