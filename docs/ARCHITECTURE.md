...
```mermaid
flowchart TD
    api["api.rs — run_toml / run_scenario · 59 kinds"]
...
```mermaid
flowchart TD
    F["api::run_toml(src) · run_scenario(src)"] --> K{"ScenarioKind::classify<br/>typed · exhaustive · 59 kinds<br/>(absent/unknown kind → clock)"}
