...
- **Telecom units.** The `telecom-timing` scenario kind reports MTIE and TDEV, checks
  ...
  G.8272.1 for an enhanced PRTC (ePRTC), G.8273.2 for a telecom boundary clock
  (T-BC) and G.8271.1 for the network limits at reference point C — and reports how long
  a holdover stays inside each time-error budget. See [`TELECOM-TIMING.md`](TELECOM-TIMING.md).
