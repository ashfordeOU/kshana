...
| **GNSS / PNT processing** | partial | **Geometry simulation only**: DOP, availability, real snapshot + solution-separation RAIM (HPL/VPL) from orbits; **RINEX 3 GPS NAV ingestion** parsed → SV ECEF position & clock bias (IS-GPS-200), and usable as a first-class `Propagator` source (broadcast ephemeris → TEME, drives the same geometry/visibility/integrity pipeline) | **Not a receiver/solver**: no pseudorange/carrier model, no PPP/RTK, no iono/tropo; GPS LNAV only (no Galileo/BeiDou/GLONASS); no SP3 yet; no RINEX-file scenario kind yet (built in code, not exposed via TOML) | multi-GNSS + SP3 + RINEX-file scenario: in progress; for real-signal processing use RTKLIB/gLAB |
