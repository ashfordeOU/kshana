...
| Interoperability & standards (RINEX/SP3/SPICE/CCSDS) | partial | RINEX 3 **multi-GNSS NAV** (GPS, Galileo, QZSS, BeiDou MEO/IGSO) ingested + propagated + runnable as a constellation; SP3-c/d precise-ephemeris **read ↔ write round trip** (parse, plus export of propagated orbits for Ginan/RTKLIB/gLAB). No GLONASS or BeiDou-GEO, no SP3 interpolation/propagator, no SPICE/CCSDS yet — **P2** "annex adjacent domains via standard formats" |
