...
**Conventions for every task:** all commands run from the repo root `/Users/chak/Documents/Code/Claudecode/spacerepo/kshana`. Commit messages use Conventional Commits and **must never** contain Claude/AI attribution (no `Co-Authored-By`, no "Generated with"). Repo git identity is already `Chakshu Baweja <contact@ashforde.org>`.
...
## Task 12: `check-no-claude` guard script

**Files:** `scripts/check-no-claude.sh` (create)

**Steps:**
1. Create `scripts/check-no-claude.sh`:
   ```bash
   #!/usr/bin/env bash
   # Fails if any Claude/AI-attribution marker appears in tracked content or commit messages.
   set -euo pipefail
   pattern='claude|anthropic|co-authored-by|generated with'
   hits=$(git grep -i -n -E "$pattern" -- . ':!scripts/check-no-claude.sh' || true)
   msgs=$(git log --format='%H %an <%ae>%n%B' | grep -i -E "$pattern" || true)
   if [ -n "$hits" ] || [ -n "$msgs" ]; then
     echo "FAIL: Claude/AI-attribution markers found:"
     [ -n "$hits" ] && echo "$hits"
     [ -n "$msgs" ] && echo "(in commit history) $msgs"
     exit 1
   fi
   echo "OK: no Claude/AI references in content or history"
   ```
2. Make executable + verify: `chmod +x scripts/check-no-claude.sh && ./scripts/check-no-claude.sh` → expect: `OK: no Claude/AI references in content or history`.
3. Commit: `git add -A && git commit -m "chore: add no-Claude content/history guard"`
...
2. Run both guards: `./scripts/check-no-claude.sh && ./scripts/check-reproducible.sh` → expect both `OK`.
...
4. Confirm clean history: `git log --oneline` shows the task commits with author `Chakshu Baweja`; `git log --format='%B' | grep -i claude` → expect: no output.
