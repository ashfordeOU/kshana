...
| **LRO lunar — validated, above 5 m (honest)** | `validated` | **Moon-centred** OD (`src/lunar_od.rs`): the GRAIL **GRGM660PRIM** gravity field (d/o 100) in the lunar body-fixed principal-axis frame (`src/lunar_frame.rs`, IAU 2015 mean-Earth + the fixed DE421 ME→PA offset) + Earth/Sun third body, fitted through the *same* generic Gauss–Newton estimator as the Earth datasets to the real **JPL Horizons LRO** (NAIF −85) reconstructed orbit, 4 h / 241 epochs, ~98 km. Honest post-fit: **dynamic 12.6 m** (RTN 3.07/10.19/6.81), **reduced-dynamic 6.6 m** (RTN 3.49/4.56/3.35, 1+2-per-rev empirical), from a 53.8 m raw overlap — **above** the < 5 m bar. The limit is the lunar orientation/ephemeris fidelity (analytic IAU libration + Montenbruck–Gill ephemeris vs DE/SPICE), not the estimator; identical at d/o 100/150, `atol` 1e-6/1e-9, and with the 2-per-rev empirical tier. Provenance + SHA-256 in `tests/fixtures/agency/NOTICE.md`; full record in `docs/REFERENCE-GRADE-OD.md`. **Validated against real agency truth: 3 of 3; meeting < 5 m: 2 of 3.** |
